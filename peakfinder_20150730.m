function varargout = peakfinder_20150730(x0, sel, thresh, extrema, include_endpoints, stdMinus)
...
if nargin < 5 || isempty(include_endpoints)
    ...
    include_endpoints = logical(include_endpoints); %boolean(include_endpoints);
end
...
if include_endpoints
    ...
    x = x0(ind);
    [minMag, minLoc]= min(x); % add this, but is temporary fix
...
%if len > 2 % Function with peaks and valleys
    ...
    tempMag = minMag;
    foundPeak = false;
    tempLoc = minLoc; % added this but is temporary fix
...
%%% main chunk
    ...
    while ii < len
        ...
        if ~foundPeak && tempMag > sel + x(ii) % would fail if x(ii) very close to tempMag
...
%%% main chunk  
    ...
    elseif ~foundPeak 
        if tempMag > (nanmedian(x0)- stdMinus*nanstd(x0)) + sel %x0(end) + sel
            peakLoc(cInd) = tempLoc;
            peakMag(cInd) = tempMag;
            cInd = cInd + 1;
        end
...
% Plot thresholds for visualization
if nargout == 0
    figure; 
    plot(1:len0,x0,'.-', peakInds, peakMags,'ro','linewidth',2); hold on;
    plot(1:len0,thresh.*ones(1,len0), 'g--'); 
    plot(1:len0,(nanmedian(x0)+sel).*ones(1,len0),'c--');
end
