...
base_dir = 'C:\Users\Ben\Documents\Data\PKA_project\20150331_SCer_KLac_YP_to_SC\'
...
fname_saveSP.SC = '20150331_processed_data_SC.mat';
fname_saveSP.KL = '20150331_processed_data_KL.mat';
...
if bgimg == 0  %set default to 1
    ...
    for jj = 1:length(ch2i)
        ...
        if strcmp(ch2i_txt,'RFP')
             imbg_jj = imread([base_dir,'BG_SC',filesep,'img_000000000_RFP_001.tif']);
        elseif strcmp(ch2i_txt,'YFP')
             imbg_jj = imread([base_dir,'BG_SC',filesep,'img_000000000_YFP_001.tif']);
...
%all locations had 4 sites
%only generated data for E1, F1, G1, and H1
%A1: Scer YP->YP 
%B1 KLac YP->YP 
%C1: Scer SC->YP 
%D1: KLac SC->YP 
...
if make_plots == 1
    ...
    %Mean NF time traces SC.MSN2 and KL.MSN2 separately
    figure(1)
    clf
    hold on
    channel = 'RFP'
    legend_vec_RFP = {'(SC.MSN2) YP->SC',
        '(SC.MSN2) SC->SC'}
    fname_save = '20150331_processed_data_SC.mat';
    ...
    perm = [1,2];
    N_RFP = length(perm);
    legend_vec_RFP_perm = legend_vec_RFP(perm);
    cmap = cmap_RFP(perm,:);
    plot_params = {'linewidth',1.5,'LineStyle','-'};
    plt_grp = zeros(length(legend_vec_RFP_perm),1);
    ...
    %perm = 1:length(legend_vec_RFP);
    perm = [1,2] ;
    legend_vec_YFP_perm = legend_vec_YFP(perm);
    cmap = cmap_YFP(perm,:);
    plot_params = {'linewidth',1.5,'LineStyle',':'};
    %plt_grp = zeros(length(legend_vec_YFP_perm),1);
    for jj = 1:length(perm)
        all_tracks = all_tracks_vec{perm(jj)};
        all_times = all_times_vec{perm(jj)};
        color_val = cmap(jj,:);
    ...
    legend_vec = [legend_vec_RFP_perm;legend_vec_YFP_perm];
    hleg = legend(plt_grp,legend_vec) %,'Location','NE');
    htitle = get(hleg,'Title');
    %set(htitle,'String','Condition')
    title('SCer cells, Media Switch')
    xlabel('time')
    ...
    hold on
    channel = 'RFP'
    legend_vec_RFP = {'(KL.MSN2) YP->SC',
        '(KL.MSN2) SC->SC'}
    fname_save = '20150331_processed_data_KL.mat';
    load([base_dir,fname_save],'all_times_vec','all_tracks_vec','posvec')
    cmap_RFP = [
        0,0,1;  %Blue
        1,0,0   %black
    ...
    wells = [1,2];
    conditions = {'YPD->SDC', 'SDC->SDC'};
    channels = {'RFP','YFP'};
    
    phase = 'Post';
    for jj = 1:length(wells)
        for kk = 1:length(channels)
            channel = channels{kk};
            subplot(2,2,jj+2*(kk-1))
            all_tracks = all_tracks_vec{jj};
            all_times = all_times_vec{jj};
            tracks = all_tracks.(phase);
            timevals = all_times.(phase);
            plot_individual_values(timevals,tracks,channel,'nf')
            axis([0,90,1.5,6.5])
            title(strcat(conditions(jj),', ',channel));
        end
    end
