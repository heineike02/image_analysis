function all_tracks = MJL_20190925_gluc_drop_osmo()
...
base_dir = 'C:\Users\BMH_work\Google Drive\UCSF\ElSamad_Lab\Jared_msn2\190925_msn2_gd2_gd8_kcl5\'
...
phases =  {'Pre', 'Post'};%, 'Post'}%,'Post_p2'} %,'Post'} 
%These are the absolute times at which each phase starts.
shift_timing = [0, 5.333];%[0,12.5]    

imdirPhase.Pre = [base_dir,'Pre_1',filesep]
imdirPhase.Post = [base_dir,'Post_1',filesep]
...
% Change this for the dim images
std_thresh = 0.15

%LED flag
analysis_params.usedLED = 0; 
...
analysis_params.thresh = 2.5; 
% standard for S. cerevisiae with Msn2 overexpressed is 2



%total number of channels of images taken
%analysis_params.channels = {'BF','RFP','YFP'};
analysis_params.channels = {'BF', 'YFP-wheel_900_gain'};

%Channels we want to process images from.

%even if it is just one channel, make it a cell
channels_to_image = {'YFP-wheel_900_gain'};
analysis_params.channels_to_image = {'YFP-wheel_900_gain'};  %{'RFP','YFP'};  
analysis_params.channels_to_image_fieldname = {'YFP'};
...
if generate_metadata_parsed ==1
    for ph = [1:length(phases)]
          ...
          %phase = ['C:\Users\BMH_work\Anaconda3\python.exe ', metadata_conv_fname, ' ', imdir]
          %system(['C:\Users\BMH_work\Anaconda3\python.exe ', metadata_conv_fname, ' ', imdir])
          system(['C:\Users\BMH_work\Anaconda3\python.exe ', metadata_conv_fname, ' "', imdir, '\"'])  %deals with space in filename, extra slash is because end of filename was escaping the quote
    end
end
%time_calc_phase.Pre =  'metadata_parsed.txt';
%time_calc_phase.Post = 'metadata_parsed.txt';
...
if bgimg == 0  %set default to 1
    ...
    for jj = 1:length(ch2i)
        ...
        elseif strcmp(ch2i_txt,'YFP-wheel_900_gain')
             imbg_jj = imread([base_dir,'BG',filesep,'img_000000000_YFP-wheel_900_gain_001.tif']);
             ch2i_fieldname = 'YFP';
            %imbg = imread('C:\Users\Ben\Box Sync\Data\PKA_Project\20140423\YFP_BG\img_000000000_Default_000.tiff');
        else
...
wellvec = {'E1','F1','G1','H1'} %,'C9','D9','E9','F9','G9','H9'};
Nsites = 4;
