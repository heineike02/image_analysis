function BMH_20140204_analysis_KL_vs_SC_2()
...
%imdirPhase.Post_ot = [base_dir,'Gluc_Drop_Post_optest\']
...
imdirPhase.Pre = [base_dir,'Gluc_Drop_Pre_2\']
imdirPhase.Post = [base_dir,'Gluc_Drop_Post_2\']
imdirPhase.Rep_ot = [base_dir,'Gluc_Rep_Post_2_optest\']
imdirPhase.Rep = [base_dir,'Gluc_Rep_Post_2\']
...
if strcmp(fname_conv,'JSO')
    ...
    for sp = 1:length(species_cell)
        ...
        for ph = 1:length(phases);
            ...
            title('SC(blue) and KL(red) Gluc Drop')
            
            figure(7)
            hold all
            Nall_tracks = length(all_tracks);
            Ntracks_vec(ph) = Nall_tracks;
            for nn = 1:Nall_tracks
